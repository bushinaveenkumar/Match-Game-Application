...
class App extends Component {
  state = {gameModeOn: true, countdownremaing: 60, Score: 0}

  changeGameMode = () =>
    this.setState(prevState => ({
      gameModeOn: !prevState.gameModeOn,
    }))
  ...
  decreasetime = () => {
    const {countdownremaing} = this.state

    if (countdownremaing > 1) {
      this.setState(prevstate => ({
        countdownremaing: prevstate.countdownremaing - 1,
      }))
    } else {
      this.setState(prevstate => ({
        countdownremaing: prevstate.countdownremaing - 1,
  ...
  render() {
    const {gameModeOn, countdownremaing, Score} = this.state

    return (
      <div className="bg-container">
        <Header countdownremaing={countdownremaing} Score={Score} />
        <div className="bottom-section">
          {gameModeOn ? (
            ...
            <div className="score-card-bg-cont">
              <img
                alt="trophy"
              ...
              <button className="play-button" onClick={this.restartGame}>
                <span>
                  <img
